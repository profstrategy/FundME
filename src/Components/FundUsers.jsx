...
const FundUsers = ({ surname, last_name, image, address, id }) => {
  ...
  return (
    <div>
      <Link to={`${id}`}>
        <ul className={`flex justify-between px-2 lg:w-10/12 w-4/6 m-auto items-center rounded-md mt-1 bg-[#1C1D24] border-y-[1px] border-stone-950 ${isSelected ? 'bg-green-500' : ''}`} onClick={() => dispatch(getProfile(id))}>
